...
import { Calendar, ChevronRight, Clock, Loader2, Search, User } from 'lucide-react'
...
const STATUS_META: Record<AppointmentStatus, { label: string; badgeClassName: string }> = {
  PROGRAMADO: {
    label: 'Programado',
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-4 text-sm text-slate-700">
          <div>
            ...
            <p className="mt-1 font-medium text-slate-900">{appointment.title}</p>
          </div>
          ...
          <div>
            ...
            <Badge className={cn('mt-1 gap-1.5 text-xs font-medium', statusMeta.badgeClassName)}>
              {statusMeta.label}
            </Badge>
          </div>
          ...
          {appointment.notes && (
            <div>
              <p className="text-xs font-medium uppercase text-slate-500">Notas</p>
          ...
          <p className="text-xs text-slate-500">
            Próximamente se agregarán acciones y herramientas adicionales para gestionar este turno.
          </p>
        </div>
      </DialogContent>
    </Dialog>
...
export default function ListaTurnosPage() {
  ...
  const filteredAppointments = useMemo(() => {
    ...
    return appointments.filter((appointment) => {
      const matchesTitle = normalizeText(appointment.title).includes(normalizedSearchTerm)
      const matchesNotes = appointment.notes
        ? normalizeText(appointment.notes).includes(normalizedSearchTerm)
        : false
      const matchesStatus = normalizeText(STATUS_META[appointment.status].label).includes(
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-600">
            Consultá los turnos agendados para cada profesional con filtros por rango de fechas y búsqueda rápida.
          </p>
        </div>
        ...
        <Card className="h-full">
          <CardHeader className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-base sm:text-lg">Profesionales</CardTitle>
                <CardDescription>
                  Filtrá y seleccioná un profesional para revisar su agenda completa.
                </CardDescription>
              </div>
            </div>
          </CardHeader>
                  ...
                  {filteredProfessionals.map((professional) => (
                    ...
                    >
                      <span className="flex flex-1 flex-col">
                        ...
                        <span className="text-xs font-normal text-slate-500">{professional.email}</span>
                      </span>
                      <ChevronRight
                        className={cn(
                          'h-4 w-4 text-slate-400 transition-transform',
                          selectedProfessional?.id === professional.id && 'translate-x-1'
                        )}
                      />
                    </Button>
                  ))}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-base sm:text-lg">
                  {selectedProfessional ? `Agenda de ${selectedProfessional.name}` : 'Agenda del profesional'}
                </CardTitle>
                <CardDescription>
                  Aplicá filtros por fechas o estado para encontrar turnos específicos.
                </CardDescription>
              </div>
            </div>
          ...
          <CardContent className="space-y-6">
            {selectedProfessional ? (
              <>
                <div className="grid gap-5 rounded-md border border-emerald-100 bg-emerald-50/40 p-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="space-y-1">
                      <p className="text-xs font-medium uppercase text-emerald-700">Fecha desde</p>
                      <DatePicker
                        date={dateFrom}
                    ...
                    <div className="space-y-1">
                      <p className="text-xs font-medium uppercase text-emerald-700">Fecha hasta</p>
                      <DatePicker
                        date={dateTo}
                ...
                {loading ? (
                  ...
                  <div className="flex flex-col items-center justify-center gap-3 rounded-md border border-dashed border-slate-200 bg-slate-50 py-16 text-center text-sm text-slate-500">
                    ...
                    <div>
                      <p className="font-medium text-slate-600">No hay turnos programados</p>
                      <p className="text-xs text-slate-500">
                        {dateFrom && dateTo
                          ? `Sin turnos entre ${dateFrom.toLocaleDateString('es-AR')} y ${dateTo.toLocaleDateString('es-AR')}`
                  ...
                  <div className="flex flex-col items-center justify-center gap-3 rounded-md border border-dashed border-slate-200 bg-slate-50 py-16 text-center text-sm text-slate-500">
                    ...
                    <div>
                      <p className="font-medium text-slate-600">No encontramos coincidencias</p>
                      <p className="text-xs text-slate-500">Revisá el término de búsqueda o ajustá los filtros.</p>
                    </div>
                  </div>
                ) : (
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <div className="flex items-center gap-2">
                                      <Badge className={cn('gap-1.5 text-xs', statusMeta.badgeClassName)}>
                                        {statusMeta.label}
                                      </Badge>
              ...
              <div className="flex flex-col items-center justify-center gap-3 rounded-md border border-dashed border-slate-200 bg-slate-50 py-16 text-center text-sm text-slate-500">
                <Calendar className="h-6 w-6 text-slate-400" />
                <div>
                  <p className="font-medium text-slate-600">Seleccioná un profesional</p>
                  <p className="text-xs text-slate-500">Elegí un profesional de la lista para ver sus turnos programados.</p>
                </div>
              </div>
            )}
          </CardContent>

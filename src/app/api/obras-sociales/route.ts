  ...
  try {
    ...
    const nuevaObra = await prisma.obraSocial.create({
      data: {
        ...
        admiteCoseguro: body.admiteCoseguro,
      },
    });
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "No se pudo crear la obra social porque el nombre ingresado ya se encuentra registrada",
        },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      { error: "Error al crear: " + errorMessage },
      { status: 500 },
    );
  }
}
